...
street_n = 1
with open("../../Chicago/street_rename/current_street_update.csv", "r") as f:
...
with open("../../Chicago/street_rename/chs_withdrawn_streets.csv", "r") as f:
    reader = csv.DictReader(f)
...
with open("../../Chicago/street_rename/chs_withdrawn_xrefs.csv", "r") as f:
    reader = csv.reader(f)
    map_id = 1
    for row in reader:
        ...
        for mapping in row[1:]:
            ...
            cur.execute(
                """INSERT INTO streetchange (id, type, note, from_id, to_id) VALUES (:id, "REPLACE", "from CHS listing", :entry_id, :to_id);""",
                d,
            )
            map_id += 1
